  ...
  // 
  // taken from three.js css3d_periodictable.html example.
  // removed the camera and rendering parts, integrated the argon versions
  //
  var objects = []
  ...
  function transform( targets, duration ) {
    // Right now, target values are copied for instant transform.
    // As a learning exercise, try to modify this code to make the values
    // tween smoothly to the next configuration (using the provided duration)
    for ( var i = 0; i < objects.length; i ++ ) {
      object = objects[ i ];
      target = targets[ i ];
